...
class PaysListView(ListView):
    ...
    def get_queryset(self):
        ...
        # Préchargement des zones avec leurs villes et localités (aperçu)
        queryset = queryset.prefetch_related(
            Prefetch(
                'zone_set',
                queryset=Zone.objects.prefetch_related('villes__localites')[:3],
        ...
        queryset = queryset.annotate(
            nb_zones=Count('zone', distinct=True),
            nb_villes=Count('zone__villes', distinct=True),
            nb_localites=Count('zone__villes__localites', distinct=True)
        )
