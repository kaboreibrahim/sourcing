...
class FournisseurExportView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Construire la requête de base avec préchargement des relations
        queryset = Fournisseur.objects.all().select_related('ville', 'ville__zone').prefetch_related('liens_commodites__commodite')
        ...
        if 'liens_commodite' in search_params:
            queryset = queryset.filter(liens_commodites__id=search_params['liens_commodite'])
        ...
        for fournisseur in queryset:
            # Récupérer les noms des commodités comme une chaîne séparée par des virgules
            noms_commodites = ", ".join(
                str(lc.commodite.nom) 
                for lc in fournisseur.liens_commodites.select_related('commodite').all()
            )
            ...
            data.append({
                ...
                'Localité': fournisseur.localite or '',
