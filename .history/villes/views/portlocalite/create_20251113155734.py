...
class VillePortCreateView(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        localite = Localite.objects.filter(localite=localite).first()
        if localite and localite.localite.zone.pays != port.pays:
            form.add_error(
                None,
        ...
        # Vérifier si la liaison existe déjà (non supprimée)
        ...
        # Créer l'instance mais ne pas encore sauvegarder
        self.object = form.save(commit=False)
        
        # Calculer et enregistrer la distance
        self.object.calculate_distance()
        
        # Sauvegarder l'objet avec la distance calculée
        self.object.save()
        
        # Afficher un message de succès avec la distance si disponible
