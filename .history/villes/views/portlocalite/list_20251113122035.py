from django.views.generic import ListView
from django.contrib.messages.views import SuccessMessageMixin
from django.urls import reverse_lazy
from django.utils.translation import gettext_lazy as _
from django.db.models import Count, Q, Prefetch
from django.contrib.auth.mixins import LoginRequiredMixin
from port.models import Port
from villes.models import PortLocalite, Ville
from zones.models import Zone
from Pays.models import Pays


# ============================================================
# VUES POUR PORT-LOCALITE (Liaison)
# ============================================================

class PortLocaliteListView(LoginRequiredMixin, ListView):
    """
    Vue pour afficher la liste des liaisons port-localite
    """
    model = PortLocalite
    template_name = 'portlocalite/port_localite_list.html'
    context_object_name = 'liaisons'
    paginate_by = 20
    def get_queryset(self):
        queryset = PortLocalite.objects.select_related(
            'port',
            'port__pays',  # Ajouté pour le filtre par pays
            'localite',
            'localite__zone',
        ).filter(deleted__isnull=True).order_by(
            'localite__nom',
            'port__nom'
        )
        
        search_query = self.request.GET.get('search', '')
        if search_query:
            queryset = queryset.filter(
                Q(port__nom__icontains=search_query) |
                Q(localite__nom__icontains=search_query) |
                Q(localite__zone__numero__icontains=search_query)
            )

        port_id = self.request.GET.get('port', '')
        if port_id:
            queryset = queryset.filter(port_id=port_id)
        
        zone_id = self.request.GET.get('zone', '')
        if zone_id:
            queryset = queryset.filter(localite__zone_id=zone_id)
            
        # Ajout du filtre par pays
        pays_id = self.request.GET.get('pays', '')
        if pays_id:
            queryset = queryset.filter(port__pays_id=pays_id)
        
        return queryset

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['search_query'] = self.request.GET.get('search', '')
        context['ports'] = Port.objects.all().order_by('nom')
        context['selected_port'] = self.request.GET.get('port', '')
        
        from zones.models import Zone
        from Pays.models import Pays  # Assurez-vous d'importer le modèle Pays
        from localites.models import Localite
        context['zones'] = Zone.objects.all().order_by('numero')
        context['selected_zone'] = self.request.GET.get('zone', '')
        
        # Ajout des villes et pays au contexte
        context['villes'] = Localite.objects.filter(deleted__isnull=True).order_by('nom')
        context['selected_ville'] = self.request.GET.get('ville', '')
        context['pays_list'] = Pays.objects.all().order_by('nom')
        context['selected_pays'] = self.request.GET.get('pays', '')
        
        context['total_liaisons'] = PortLocalite.objects.filter(
            deleted__isnull=True
        ).count()
        
        return context