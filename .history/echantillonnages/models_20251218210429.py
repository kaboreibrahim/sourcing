...
class Echantionnage(SafeDeleteModel):
    ...
    # Mise à jour de la méthode save pour générer la référence
    def save(self, *args, **kwargs):
        if not self.reference:
            self.reference = f"ECH-{self.id}"
        super().save(*args, **kwargs)
        # Mise à jour de la quantité disponible du fournisseur
        self.update_fournisseur_quantite()
    
    def delete(self, *args, **kwargs):
        # Sauvegarder la référence pour la mise à jour après suppression
        fournisseur_commodite = self.fournisseur_commodite
        super().delete(*args, **kwargs)
        # Mettre à jour la quantité après suppression
        fournisseur_commodite.update_quantite()
    
    def update_fournisseur_quantite(self):
        """Met à jour la quantité disponible du fournisseur pour cette commodité"""
        if self.fournisseur_commodite:
            self.fournisseur_commodite.update_quantite()
    
    # Méthodes utilitaires
    @property
    def fournisseur(self):
        """Retourne le fournisseur associé"""
        return self.fournisseur_commodite.fournisseur if self.fournisseur_commodite else None
    
    @property
    def quantite_disponible(self):
        """Retourne la quantité disponible pour cette commodité du fournisseur"""
        return self.fournisseur_commodite.quantite_disponible if self.fournisseur_commodite else 0

    # Signal pour mettre à jour la quantité lors de la création/mise à jour
    @receiver(post_save, sender=Echantionnage)
    def update_quantite_on_save(sender, instance, **kwargs):
        if instance.fournisseur_commodite:
            instance.fournisseur_commodite.update_quantite()
