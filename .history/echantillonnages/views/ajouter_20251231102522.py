...
class AjouterEchantillonView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        # URL de redirection par défaut pour les requêtes non-AJAX
        return reverse('detail_commodite', kwargs={'pk': self.object.commodite.id})
            
        # echantillonnages/views/ajouter.py
    def form_valid(self, form):
        # Ne pas sauvegarder le champ commission
        if 'commission' in form.cleaned_data:
            del form.cleaned_data['commission']
        
        self.object = form.save(commit=False)
        if self.request.user.is_authenticated:
            self.object.utilisateur = self.request.user
        if hasattr(self.request.user, 'fournisseur_profile'):
            self.object.fournisseur = self.request.user.fournisseur_profile
        self.object.save()
        ...
        if self.request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({
                ...
                'redirect_url': reverse('detail_commodite', kwargs={'pk': self.object.commodite.id})
            })
...
class ModifierEchantillonView(LoginRequiredMixin, UpdateView):
    ...
    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        kwargs['user'] = self.request.user
        if hasattr(self.object, 'commodite'):
            kwargs['commodite'] = self.object.commodite
        return kwargs
    
    def get(self, request, *args, **kwargs):
        self.object = self.get_object()
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            # Retourner les données de l'échantillon au format JSON
            return JsonResponse({
                'success': True,
                'echantillon': {
                   'quantite': str(self.object.quantite),
                   'prix': str(self.object.prix),
                   'date_creation': self.object.date_creation.strftime('%Y-%m-%dT%H:%M'),
