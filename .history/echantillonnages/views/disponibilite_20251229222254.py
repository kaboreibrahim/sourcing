...
class ListeDisponibilitesView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Sous-requête pour obtenir le dernier échantillon de chaque fournisseur par commodité
        derniers_echantillons = Echantionnage.objects.filter(
            id__in=Subquery(
        ...
        for commodite in commodites:
            ...
            quantite_totale = echantillons_commodite.aggregate(
                total=Sum('quantite')
            )['total'] or 0
            ...
            if fournisseurs_data:  # Ne pas inclure les commodités sans fournisseurs
                disponibilites.append({
                    'commodite': commodite,
                    'quantite_totale': quantite_totale,
                    'nombre_fournisseurs': echantillons_commodite.count(),
