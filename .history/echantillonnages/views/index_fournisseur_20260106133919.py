...
from django.db.models import Prefetch, Avg, F, FloatField, Case, When, Value, IntegerField, Sum
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .prefetch_related(
                ...
                Prefetch(
                    'liens_fournisseurs',
                    queryset=FournisseurCommodite.objects.filter(fournisseur=fournisseur)
                )
            )
            .annotate(
                quantite_totale=Coalesce(
                    Sum('liens_fournisseurs__quantite_disponible', 
                        filter=F('liens_fournisseurs__quantite_disponible__isnull').bitor(
                               F('liens_fournisseurs__quantite_disponible').isnull(False)
                        ),
                ...
                prix_moyen=Coalesce(
                    Avg('liens_fournisseurs__prix_unitaire',
                        filter=F('liens_fournisseurs__prix_unitaire__isnull').bitor(
                               F('liens_fournisseurs__prix_unitaire').isnull(False)
                        ),
                        output_field=FloatField()
                    ),
                    Value(0.0, output_field=FloatField())
