...
class DemandeCotationCreateModalView(DemandeCotationCreateView):
    """Vue pour charger le formulaire de création dans une modale"""
    
    def get(self, request, *args, **kwargs):
        form = self.get_form()
        context = self.get_context_data(form=form)
        form_html = render_to_string('cotations/partials/cotation_form.html', context, request=request)
    ...
    def form_invalid(self, form):
        context = self.get_context_data(form=form)
        form_html = render_to_string('cotations/partials/cotation_form.html', context, request=self.request)
        return JsonResponse({
            'success': False,
            'form_html': form_html
        })  
