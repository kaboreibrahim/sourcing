    ...
    def handle(self, *args, **options):
        ...
        for email_item in queryset:
            try:
                ...
                # Créer l'email
                # Préparer la liste des destinataires en copie (CC)
                cc_emails = []
                if email_item.cc_emails:
                    # Nettoyer et formater les adresses CC
                    if email_item.cc_emails.startswith('[') and email_item.cc_emails.endswith(']'):
                        # Si la chaîne ressemble à une liste Python, la convertir en liste
                        import ast
                        try:
                            cc_list = ast.literal_eval(email_item.cc_emails)
                            if isinstance(cc_list, list):
                                cc_emails = [email.strip() for email in cc_list if isinstance(email, str) and email.strip()]
                        except (ValueError, SyntaxError):
                            # Si la conversion échoue, essayer de diviser par des virgules
                            cc_emails = [email.strip() for email in email_item.cc_emails.split(',') if email.strip()]
                    else:
                        # Diviser par des virgules par défaut
                        cc_emails = [email.strip() for email in email_item.cc_emails.split(',') if email.strip()]
                
                # Créer l'email avec les destinataires en copie
                email = EmailMessage(
                    subject=email_item.subject,
                    body=email_item.html_content,
                    from_email=settings.DEFAULT_FROM_EMAIL,
                    to=[email_item.to_email],
                    cc=cc_emails if cc_emails else None,  # Ne pas inclure cc si la liste est vide
                )
