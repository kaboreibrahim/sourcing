...
def upload_to_utilisateur_profile(instance, filename):
    """Génère le chemin d'upload pour les photos de profil des artisans"""
    ext = filename.split('.')[-1]
    filename = f"{instance.id}.{ext}"
    return f"artisans/{instance.user.username}/profil/{filename}"
...
class Utilisateur(AbstractUser, SafeDeleteModel):
    ...
    photo_profil = models.ImageField(upload_to=upload_to_artisan_profile, blank=True)
