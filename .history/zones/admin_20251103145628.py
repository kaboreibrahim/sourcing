...
class PaysListFilter(SimpleListFilter):
    title = _('Pays')
    parameter_name = 'pays'

    def lookups(self, request, model_admin):
        qs = Zone.objects.values('pays').distinct()
        return [(pays.id, pays.pays.nom) for pays in qs]

    def queryset(self, request, queryset):
        if self.value():
            return queryset.filter(pays_id=self.value())
