        ...
        return Zone.objects.prefetch_related(
            Prefetch(
                ...
                queryset=Ville.objects.filter(deleted__isnull=True)
                    ...
                    .prefetch_related(
                        Prefetch(
                            'localites',  # ✅ nom correct de la relation
                            queryset=Localite.objects.filter(deleted__isnull=True)
                        )
                    )
                    .annotate(num_localites=Count('localites', distinct=True))  # ✅ pluriel
                    .annotate(num_fournisseurs=Count('localites__fournisseurs', distinct=True))  # ✅ pluriel
            ),
            'villes__localites__fournisseurs',  # ✅ pluriel
            'villes__localites__fournisseurs__liens_commodites__commodite',  # ✅ pluriel
        )
