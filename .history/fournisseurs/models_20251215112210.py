...
class Fournisseur(SafeDeleteModel):
    ...
    def __str__(self):
        return f"{self.nom} ({self.localite or 'Sans localité'})"


class FournisseurPort(SafeDeleteModel):
    """
    Modèle de liaison entre un Fournisseur et un Port.
    Permet de gérer plusieurs ports pour un même fournisseur.
    """
    _safedelete_policy = SOFT_DELETE_CASCADE
    ...
    port = models.ForeignKey(
        'port.Port',
        on_delete=models.CASCADE,
        related_name='fournisseurs_lies',
        verbose_name=_("Port")
    )
    ...
    distance = models.DecimalField(
        ...
        blank=True,
        null=True,
        help_text=_("Distance entre le fournisseur et le port en kilomètres")
    )
    ...
    class Meta:
        ...
        verbose_name_plural = _("Liens Fournisseurs-Ports")
