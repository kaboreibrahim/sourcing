...
class FournisseurExportView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if 'responsable' in search_params:
            queryset = queryset.filter(responsable__icontains=search_params['responsable'])
        ...
        for fournisseur in queryset:
            data.append({
                ...
                'Téléphone': fournisseur.telephone or '',
                ...
                'Date création': fournisseur.date_creation.strftime('%d/%m/%Y %H:%M') if fournisseur.date_creation else '',
                'Dernière modification': fournisseur.date_modification.strftime('%d/%m/%Y %H:%M') if fournisseur.date_modification else ''
            })
