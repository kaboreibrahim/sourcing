...
class PortFournisseurCreateView(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    form_class = FournisseurPortForm  # Ajoutez cette ligne
    ...
    def form_valid(self, form):
        # Si le formulaire est valide mais que le fournisseur est désactivé, on le réactive
        if 'fournisseur' in form.cleaned_data and hasattr(form.fields['fournisseur'], 'widget') and 'disabled' in form.fields['fournisseur'].widget.attrs:
            form.instance.fournisseur = get_object_or_404(Fournisseur, id=self.kwargs['fournisseur_id'])
        ...
        if not form.cleaned_data.get('distance'):
            try:
                ...
                form.instance.calculate_distance()
            except Exception as e:
                messages.warning(
                    self.request,
                    _("La distance n'a pas pu être calculée automatiquement. Veuillez la saisir manuellement.")
                )
                return self.form_invalid(form)
