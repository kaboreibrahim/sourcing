...
class AccueilView(ListView):
    ...
    def get_queryset(self):
        # Récupérer l'ID de la commodité depuis l'URL ou utiliser une valeur par défaut
        self.commodite_id = self.kwargs.get('commodite_id')
        
        if not self.commodite_id:
            # Si aucun commodite_id n'est fourni, retourner une liste vide
            return Echantionnage.objects.none()
        ...
        except (ValueError, Commodite.DoesNotExist):
            return Echantionnage.objects.none()
    ...
    def get_context_data(self, **kwargs):
        ...
        # Récupérer toutes les commodités pour le menu déroulant
        ...
        context.update({
            ...
            'titre_page': f"Disponibilité - {getattr(getattr(self, 'commodite', None), 'nom', 'Sélectionnez une commodité')}"
        })
